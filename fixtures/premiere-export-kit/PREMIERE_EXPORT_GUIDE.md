...
Sequence 시작은 `00:00:00:00`, 전체 길이는 정확히 13초로 만듭니다.
...
| V1 | 00:12 | 00:13 | 검정 `Color Matte` | Sequence 끝을 1초 연장 |
| V2 | 00:02 | 00:05 | `overlay-d.mp4` | 앞 3초 사용, Overlay 합성 모드 적용 후 위쪽으로 이동 |
| V2 | 00:07 | 00:08 | `disabled-e.mp4` | 1초만 사용한 뒤 우클릭 `Enable` 해제 |
| V3 | 00:00 | 00:12 | `Adjustment Layer` | 일반적인 Premiere Adjustment Layer 제외 검증 |

간단히 보면 다음 구조입니다.

```text
TIME  00      02  03      05  06  07  08  09      12 13
V3    [            ADJUSTMENT LAYER                  ]
...
이 실제 Premiere export fixture는 위에 지정한 Overlay 합성·위치, Adjustment Layer, Color Matte 외에는 아래 항목을 추가하지 않습니다.

- Cross Dissolve 또는 다른 transition
- 좌우 반전, 추가 Transform, Crop 등의 effect/filter
...
- [ ] 최종 영상 길이가 13초인가
- [ ] 최종 영상이 24 fps인가
- [ ] 2–5초에 위로 이동한 `OVERLAY D`가 아래 CLIP A/B와 합성되어 보이는가
- [ ] 7–8초에 노란색 `DISABLED E`가 보이지 않고 빨간색 `CLIP A`가 보이는가
- [ ] 12–13초에 검정 Color Matte가 보이는가
- [ ] XML과 최종 MP4의 Sequence 길이가 같은가
- [ ] 지정된 합성·Adjustment Layer·Color Matte 외의 transition, effect, mask, speed change를 넣지 않았는가
