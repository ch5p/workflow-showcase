...
remux 결과 영상은 그대로 12초·288프레임이지만 AAC 컨테이너 끝길이는 `12.010667초`입니다. 이 fixture는 시각 기준이므로 영상 타임라인 contract에는 영향이 없으며, 공개 MP4는 원본과 바이트 단위로 같은 파일이 아닙니다.

## MP4 참고

- Video: H.264 Main, 1280×720, yuv420p, progressive, 24 fps, 288 frames
- Audio: AAC-LC, 48 kHz, stereo, 컨테이너 기준 12.010667초
...
최종 시각 검수는 2026-07-16 maintainer 확인으로 `Passed`입니다. 자동 metadata/parser 검증과 별도로 아래 화면과 복원 동선을 직접 확인했습니다.

- [x] 2–5초 구간에 보라색 `OVERLAY D`가 보임
- [x] 7–8초 구간에 비활성 노란색 `DISABLED E`가 보이지 않고 빨간색 `CLIP A`가 보임
- [x] 전체 길이 12초 동안 A → D → B → A → C 순서와 최종 MP4 화면이 일치함
- [x] NEW JOB 불러오기와 기존 Job 폴더 복사 복원이 정상 동작함
...
| Raw MP4 (not published) | `045D67722997455E424CCA423978DEDC8B39AB7C4E364F0A12567C78E08589F2` |
| Sanitized public MP4 | `AF08A73365FCB7B892D7B46B198C9FBD536F8819F57B4E6B0F8BA29AE7275454` |
