...
> `Workflow Showcase template`은 Seedance 2.0 같은 AI 생성 도구로 영상을 만드는 사람들이 제작에 사용한 레퍼런스와 프리미어·다빈치 리졸브에서 내보낸 XML 또는 실험적으로 지원하는 CapCut Desktop 9.x 프로젝트 데이터를 기반으로, 최종 렌더링된 영상 밑에 간단한 워크플로우를 손쉽게 얹어주는 템플릿 앱입니다.

**목차**: [AI 커스터마이징](#ai로-자유롭게-바꿔-쓰세요) · [미리보기](#미리보기) · [설치 방법](#설치-방법) · [사용 방법](#사용-방법) · [입력값 설정](#입력값-설정) · [레퍼런스 세팅](#레퍼런스-세팅하는-방법) · [문제 해결](#문제-해결) · [마치며](#마치며)

## AI로 자유롭게 바꿔 쓰세요

기본 상태로 바로 사용할 수 있고, 거기서 멈출 필요도 없습니다. 레이아웃·콜아웃·화면 비율·레퍼런스 배치까지 자기 작업 방식과 취향에 맞게 자유롭게 바꿔보세요. Codex·Claude 같은 AI 에이전트가 필요한 부분을 빠르게 찾아 수정할 수 있도록 코드 구조와 전용 커스터마이징 가이드를 준비했습니다. 이 저장소를 자신만의 Workflow Showcase로 만들어 쓰세요.
...
처음 켜면 샘플 Job을 재생할 수 있고, 본인 타임라인을 넣고 교체하시면 됩니다.
...
## 1. 프리미어·다빈치 리졸브 또는 CapCut에서 타임라인 데이터를 준비하세요.

- 프리미어는 내보내기 > Final Cut Pro XML...로 XML 파일을 출력하세요.
- 다빈치 리졸브는 타임라인을 Final Cut Pro 7 XML로 출력하세요. DaVinci Resolve 21.0.2에서 실제 작동을 확인했습니다.
- 이 앱은 Adjustment Layer 같은 효과 레이어를 현재 지원하지 않으며 XML을 불러올 때 자동으로 제외합니다. 기본 UI에는 실제 영상 클립만 표시됩니다. 효과 레이어를 `FX`처럼 노출하려면 별도의 입력 어댑터와 UI를 추가해야 합니다.
- CapCut 입력은 실험 기능입니다. Windows CapCut Desktop 9.x 로컬 프로젝트를 저장하고 해당 프로젝트에서 나가거나 CapCut을 닫은 뒤 `TIMELINE > CAPCUT PROJECT · EXPERIMENTAL`에서 프로젝트 폴더를 선택하세요. XML을 따로 뽑을 필요는 없습니다.

## 2. 앱을 실행한 뒤에 TIMELINE과 VIDEO를 각각 드래그&드롭 또는 클릭으로 집어넣습니다.

- `TIMELINE`을 누르고 XML 또는 실험용 CapCut 프로젝트 입력을 선택하세요. TIMELINE과 VIDEO를 한 번에 넣는 기능은 현재 미지원입니다.
- 완성본 VIDEO 입력은 재생 가능한 H.264 MP4만 지원합니다. MOV/ProRes와 M4V는 최종 영상으로 불러올 수 없습니다. 편집 프로그램에서 오디오를 AAC로 설정한 H.264 MP4를 출력해 넣어주세요.

![TIMELINE·VIDEO 불러오기](./docs/readme-assets/animations/import-xml-video.webp)
...
불러온 타임라인과 완성본 영상 길이가 1프레임 이상 다르면, 정지 상태에서 영상 왼쪽 위에 `DURATION Δ` 배지가 뜹니다. 입력에 따라 `XML` 또는 `CAPCUT`으로 표시됩니다.
...
위 프리미어 예시는 **XML에 등록된 끝은 45.79초인데 실제 비디오 길이는 48.79초다**를 알려 주는 거고 출력하면 불러온 타임라인을 따라 실제로 **45.79초까지만 렌더링**됩니다. 해서 `영상은 10초에 끝났지만 1초의 여운을 남기고 싶다`면 색상 매트로 원하는 타임까지 잡고 XML로 내보내야 합니다. CapCut은 프로젝트 길이를 수정·저장한 뒤 타임라인을 다시 불러오면 됩니다.
...
CapCut 입력에서는 같은 역할의 **UPDATE TIMELINE**이 표시됩니다. XML과 CapCut 형식을 서로 바꿀 때는 NEW JOB으로만 교체됩니다.

**NEW JOB**은 작업이 끝나고 새로운 작업물로 갈아끼울 때 사용합니다.

현재 `current-job` 폴더 하나만 운영합니다. 별도의 마이그레이션 코드는 과하다 생각해 구현하지 않았습니다. 렌더링된 최종결과물은 `current-job` 외부의 `output`에 별도 보관됩니다. 새 Job으로 바꾸면 현재 작업 세팅은 초기화되며, 가장 귀찮게 다시 해야 하는 건 샷별 레퍼런스 연결입니다. (노가다 다시 하는 정도)

![BACKUP JOB](./docs/readme-assets/images/backup-job.webp)

EDIT PANEL 하단에 작은 BACKUP JOB 버튼이 있습니다. 등록된 타임라인과 연결된 레퍼런스를 나중에라도 **재활용할지도 모르니 보존하고 싶다** 하면 이용하세요. 백업에는 원본 영상과 렌더링 결과물이 포함되지 않으며, 앱 안에서 바로 복원하는 기능도 제공하지 않습니다.
...
**영상은 재생되는데 Export에서 오디오 오류가 나요**
→ 원본 오디오는 변환하지 않고 그대로 복사합니다. 현재 검증된 건 AAC뿐입니다. 실패하면 오디오를 AAC로 설정한 H.264 MP4로 다시 출력해 넣어주세요.

**렌더링 결과가 예상보다 짧아요**
→ 위의 [DURATION Δ](#길이가-안-맞을-때-duration-δ) 항목 참고. 불러온 타임라인 길이만큼만 렌더링됩니다.
...
- 안정적으로 지원하는 입력은 Final Cut Pro 7 XML(`xmeml`)입니다. Adobe Premiere Pro와 DaVinci Resolve 21.0.2에서 내보낸 XML의 실제 작동을 확인했습니다. VEGAS Pro도 Final Cut Pro 7/DaVinci Resolve XML을 출력할 수 있어 작동 가능성이 있지만 아직 확인하지 않았습니다. Windows CapCut Desktop 9.x 로컬 프로젝트는 실험 기능으로 지원하며 원본 draft와 절대 미디어 경로는 저장하지 않습니다. 최신 Final Cut Pro의 `.fcpxml`은 현재 지원하지 않으므로 [입력 어댑터 계약](./docs/INPUT_ADAPTER_CONTRACT.md)을 따르는 별도 어댑터가 필요합니다.
...
- DaVinci Resolve 21.0.2의 Final Cut Pro 7 XML

추가 호환성 안내입니다.

- 현재 Beta 앱은 Windows 전용이며 macOS 실행은 지원하지 않습니다.
- VEGAS Pro의 Final Cut Pro 7/DaVinci Resolve XML은 작동 가능성이 있지만 아직 실제 프로젝트로 확인하지 않았습니다.
