...
- `main.cjs`: Electron window and IPC composition, TIMELINE file/project selection, UPDATE/NEW JOB decision boundary, `job.json` CAS save, and app log
...
- `preload.cjs`: the minimal local file API exposed to the screen, plus restricted IPC for timeline/video/reference drop
- `timeline-input.cjs`: the two supported input descriptors, canonical candidate/source names, legacy `job.xml` resolver, and Job-field compatibility helper
- `job-lifecycle.cjs`: format-aware prepare/commit/rollback/crash recovery and Windows-safe staged replace for timeline UPDATE/NEW JOB
...
- `job-backup.cjs`: manual dated snapshot of the complete `job.json` (including optional `introPreroll`), the recorded timeline input, registered references, and a hash manifest; source video, Export/demo output, and logs are excluded
...
- `src/adapters/capcut-draft-parser.js`: experimental CapCut Desktop 9.x parser and sanitized app-owned snapshot validator; it never persists raw media paths
...
- `timelineInput`: optional common descriptor containing `format`, `name`, `relativePath`, and an optional editor version. `format` is `"xmeml"` or experimental `"capcut-draft"`. Existing version-1 Jobs that contain only `xml` resolve in memory as xmeml and are not force-migrated. New xmeml Jobs retain the matching `xml` compatibility record; CapCut Jobs keep `xml: null`.
...
- `referenceMotion`: optional `"classic"` or `"pop3d"`, selected by `REFERENCE 3D POP` inside `EDIT DISPLAY`. Absent means `"classic"`. Preview and offscreen Export use the same value; NEW JOB restores the default while same-format timeline UPDATE preserves it.
- `editNumberTicker`: optional boolean selected by `NUMBER TICKER` inside `EDIT DISPLAY`, alongside `REFERENCE 3D POP`. Absent means `false`; `true` animates the EDIT number at cut boundaries during continuous playback. Preview and offscreen Export use the same value; NEW JOB restores `false` while same-format timeline UPDATE preserves it.
- `introPreroll`: optional top-level object containing only `prompt`, `reply`, `typingSeconds`, and `soundEnabled`. Missing settings normalize to the shipped prompt/reply defaults, `typingSeconds: 1`, and `soundEnabled: true`; the only accepted typing choices are `1` and `2`. Same-format timeline UPDATE preserves it, NEW JOB restores the defaults, and BACKUP JOB includes it through the complete `job.json` snapshot.
...
- `current-job/source/timeline.xml`: the xmeml imported into the app
- `current-job/source/timeline.capcut.json`: sanitized normalized snapshot created from a selected CapCut `draft_content.json`; the raw draft and absolute paths are not stored
...
- `backup/<YYYY-MM-DD_HH-mm-ss>/`: manual settings snapshots containing the complete `job.json`, recorded timeline input, recorded references, and a hash manifest; source video, normal Export/INTRO output, and logs are excluded

All stored Job paths are relative to the `current-job` root. Store `source/timeline.xml` or `source/timeline.capcut.json`, never an absolute path or a `current-job/...`-prefixed path. Internal identifiers and JSON keys are not changed.
...
- The stable production adapter accepts legacy Final Cut Pro 7 XML (`xmeml`). Premiere Pro and DaVinci Resolve 21.0.2 exports are validated; VEGAS Pro exports the same named interchange format but remains unverified. The experimental second adapter accepts a user-selected local Windows CapCut Desktop 9.x project folder or its `draft_content.json`. Both follow `docs/INPUT_ADAPTER_CONTRACT.md`; neither rewrites the other parser or the PRIMARY/SHOT core.
- The `TIMELINE` click/drop zone accepts xmeml, `draft_content.json`, or a CapCut project folder. Click first chooses `XML` or `CAPCUT PROJECT · EXPERIMENTAL`. Same-format imports offer UPDATE/NEW JOB/CANCEL; switching formats requires an explicit NEW JOB because source identity is editor-specific.
...
- A valid timeline selected while `demo: true` bypasses UPDATE and enters the existing NEW JOB transaction, so the disposable sample XML/video are removed while Current Job logs remain and app-root Export files are unaffected. The candidate is parsed before this decision. Ordinary Jobs keep the normal same-format UPDATE/NEW JOB dialog.
- On selection cancel, file-pick cancel, or validation failure, the existing `job.json`, source, and references are unchanged.
- UPDATE replaces only the active same-format timeline file and preserves video/reference/GLOBAL/title/callout/`referenceMotion`/`editNumberTicker`/`introPreroll`/`ui`/`output`. It prefers exact source identity, falls back only when there is unique name + source range/occurrence evidence, and keeps ambiguous/unmatched mappings as orphans.
- NEW JOB cleans up source timeline/video and reference files when explicitly chosen for an ordinary Job, or automatically after a valid timeline is selected for `demo: true`. It resets `references`, `globalReferenceIds`, previous `shotMappings`/orphans, `projectTitle`, `callout`, `referenceMotion`, `editNumberTicker`, and `introPreroll`. It stores anonymous descriptors in `timelineShots`, preserves `current-job/logs/`, existing `ui`, and Job `output` settings, and cannot delete the separate app-root `output/` files.
...
- Every mutation compares `jobId + revision`, so an earlier debounced save arriving after a timeline UPDATE or NEW JOB cannot overwrite the current state.
- Timeline/video transactions retry Windows rename lock errors 4 times, and on persistent failure replace the manifest and Job files via durable copy, fsync, and SHA-256 verification. A valid primary manifest is always the reference; a staging manifest is used only when the primary is missing or corrupted.
...
- `fixtures/capcut-9x/public-fixture/draft_content.json`: minimized, path-sanitized CapCut Desktop 9.0.0 structure with A/B/A on the base track and C on the upper track
- `fixtures/capcut-9x/SOURCE_NOTES.md`: CapCut fixture provenance and the expected 186-frame, A/C/B/A, 4-EDIT, 3-SHOT contract
...
Before the Export popup shows `READY`, `main.cjs` resolves the stored timeline input, video, and every registered reference with `mustExist: true` inside its owned Current Job directory. The actual Export start repeats its own source/reference checks; the readiness display does not replace that final guard.

After the timeline duration is known and before FFmpeg starts, Export estimates the video payload from duration and selected bitrate, adds a 512 kbps audio allowance, 5% container overhead, and the same bounded safety reserve. Insufficient free space fails before encoding with `INSUFFICIENT_DISK_SPACE`; a successful check records `export_space_checked`.
...
- Code assumption: `main.cjs`'s `JOB_ROOT`, `durable-file.cjs`, `owned-path.cjs`, the timeline/video lifecycles, timeline reconcile, `src/core/*`, `render-spec.cjs`, the CAS guard, `hydrateJob()`, and the isolated `intro-demo-controller.cjs`/`src/intro-preroll.html` path all read the same structure and ownership boundary.
