...
| 🔴 **Report + verify yourself / avoid** | **Save, path checks, XML/video import**, the **encoder/codec contract**, the **callout timing driven by video time**, the **XML parser and PRIMARY calculation** (`src/core`) | No. Getting this wrong can silently break data or output. |
