...
| 🔴 **Report + verify yourself / avoid** | **Save, path checks, timeline/video import**, the **encoder/codec contract**, INTRO **source selection/FFmpeg/concat/finalization**, the **callout timing driven by video time**, the **xmeml/CapCut adapters and PRIMARY calculation** (`src/core`) | No. Getting this wrong can silently break data or output. |
...
| Change INTRO conversation, typing time, or sound | "Keep the existing `introPreroll` schema and edit only prompt, reply, the 1/2-second typing choice, or SOUND ON/OFF" | 🟡 Do not add selected Export or asset paths to `job.json`. Same-format timeline UPDATE preserves these settings; NEW JOB resets them. |
